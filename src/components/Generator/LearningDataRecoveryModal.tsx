...
const getReportProfile = (gameName: string, caseKey: ReportCaseKey): ReportProfile => {
  if (caseKey === 'bilingual') {
    return {
      ...
      title: '双语课堂闯关',
      student: '一年级二班 · 王小雨',
      overview: '汇总孩子在「双语课堂闯关」互动课件中的表现，覆盖 1 个班级、3 个任务环节和 5 个错题关卡。',
      tags: ['双语理解', '课堂反应', '错题巩固'],
  ...
  if (caseKey === 'level') {
    return {
      ...
      title: '颜色单词闯关',
      student: '学生：王小雨 · 本次练习',
      overview: '汇总孩子在「颜色单词闯关」互动课件中的表现，覆盖 4 个关卡、10 道图词匹配和听音选择题。',
      tags: ['颜色识别', '听音辨词', '图词匹配'],
  ...
  return {
    ...
    title,
    student: '学生：王小雨 · 本次练习',
    overview: `汇总孩子在「${title}」互动课件中的表现，覆盖 3 个关卡、12 道动物单词选择题。`,
    tags: ['动物识别', '听音辨词', '图词匹配'],
